...
WallClass::WallClass(House* newOwner) : StructureClass(newOwner)
{
    WallClass::init();
...
WallClass::WallClass(Stream& stream) : StructureClass(stream)
{
    ...
    bWallDestroyedUp = stream.readBool();
    bWallDestroyedRight = stream.readBool();
    bWallDestroyedDown = stream.readBool();
    bWallDestroyedLeft = stream.readBool();
...
void WallClass::save(Stream& stream) const
{
	StructureClass::save(stream);

    stream.writeBool(bWallDestroyedUp);
    stream.writeBool(bWallDestroyedRight);
    stream.writeBool(bWallDestroyedDown);
    stream.writeBool(bWallDestroyedLeft);

    // save current wall tile
    stream.writeSint32(curAnimFrame);
}

void WallClass::destroy()
{
	// fix wall to the north
	if(currentGameMap->cellExists(location.x, location.y-1) == true) {
        ObjectClass* obj = currentGameMap->getCell(location.x, location.y-1)->getGroundObject();
        if((obj != NULL) && (obj->getItemID() == Structure_Wall)) {
            ((WallClass*) obj)->bWallDestroyedDown = true;
            ((WallClass*) obj)->fixWall();
        }
	}
	...
	if(currentGameMap->cellExists(location.x, location.y+1) == true) {
        ObjectClass* obj = currentGameMap->getCell(location.x, location.y+1)->getGroundObject();
        if((obj != NULL) && (obj->getItemID() == Structure_Wall)) {
            ((WallClass*) obj)->bWallDestroyedUp = true;
            ((WallClass*) obj)->fixWall();
        }
	}

	// fix wall to the west
	if(currentGameMap->cellExists(location.x-1, location.y) == true) {
        ObjectClass* obj = currentGameMap->getCell(location.x-1, location.y)->getGroundObject();
        if((obj != NULL) && (obj->getItemID() == Structure_Wall)) {
            ((WallClass*) obj)->bWallDestroyedRight = true;
            ((WallClass*) obj)->fixWall();
        }
	}
	...
	if(currentGameMap->cellExists(location.x+1, location.y) == true) {
        ObjectClass* obj = currentGameMap->getCell(location.x+1, location.y)->getGroundObject();
        if((obj != NULL) && (obj->getItemID() == Structure_Wall)) {
            ((WallClass*) obj)->bWallDestroyedLeft = true;
            ((WallClass*) obj)->fixWall();
        }
	}
...
*/
void WallClass::setLocation(int xPos, int yPos) {
    StructureClass::setLocation(xPos, yPos);
	...
	if(currentGameMap->cellExists(location.x, location.y-1) == true) {
        ObjectClass* obj = currentGameMap->getCell(location.x, location.y-1)->getGroundObject();
        if((obj != NULL) && (obj->getItemID() == Structure_Wall)) {
            ((WallClass*) obj)->bWallDestroyedDown = false;
            ((WallClass*) obj)->fixWall();
        }
	}
	...
	if(currentGameMap->cellExists(location.x, location.y+1) == true) {
        ObjectClass* obj = currentGameMap->getCell(location.x, location.y+1)->getGroundObject();
        if((obj != NULL) && (obj->getItemID() == Structure_Wall)) {
            ((WallClass*) obj)->bWallDestroyedUp = false;
            ((WallClass*) obj)->fixWall();
        }
	}
	...
	if(currentGameMap->cellExists(location.x-1, location.y) == true) {
        ObjectClass* obj = currentGameMap->getCell(location.x-1, location.y)->getGroundObject();
        if((obj != NULL) && (obj->getItemID() == Structure_Wall)) {
            ((WallClass*) obj)->bWallDestroyedRight = false;
            ((WallClass*) obj)->fixWall();
        }
	}
	...
	if(currentGameMap->cellExists(location.x+1, location.y) == true) {
        ObjectClass* obj = currentGameMap->getCell(location.x+1, location.y)->getGroundObject();
        if((obj != NULL) && (obj->getItemID() == Structure_Wall)) {
            ((WallClass*) obj)->bWallDestroyedLeft = false;
            ((WallClass*) obj)->fixWall();
        }
	}
}
...
void WallClass::fixWall() {
    ...
    // Walls
    bool up = (currentGameMap->cellExists(i, j-1) && (currentGameMap->cell[i][j-1].hasAGroundObject()
                && (currentGameMap->cell[i][j-1].getGroundObject()->getItemID() == Structure_Wall))) || bWallDestroyedUp;

    bool right = (currentGameMap->cellExists(i+1, j) && (currentGameMap->cell[i+1][j].hasAGroundObject()
                && (currentGameMap->cell[i+1][j].getGroundObject()->getItemID() == Structure_Wall))) || bWallDestroyedRight;

    bool down = (currentGameMap->cellExists(i, j+1) && (currentGameMap->cell[i][j+1].hasAGroundObject()
                && (currentGameMap->cell[i][j+1].getGroundObject()->getItemID() == Structure_Wall))) || bWallDestroyedDown;

    bool left = (currentGameMap->cellExists(i-1, j) && (currentGameMap->cell[i-1][j].hasAGroundObject()
                && (currentGameMap->cell[i-1][j].getGroundObject()->getItemID() == Structure_Wall))) || bWallDestroyedLeft;

    // calculate destroyed tile index
    int destroyedTileIndex = 0;
    if(left == true)
        destroyedTileIndex = (destroyedTileIndex << 1) | bWallDestroyedLeft;

    if(down == true)
        destroyedTileIndex = (destroyedTileIndex << 1) | bWallDestroyedDown;

    if(right == true)
        destroyedTileIndex = (destroyedTileIndex << 1) | bWallDestroyedRight;

    if(up == true)
        destroyedTileIndex = (destroyedTileIndex << 1) | bWallDestroyedUp;
